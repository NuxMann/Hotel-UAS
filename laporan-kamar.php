...
<head>
  ...
  <title>Data Laporan Kamar</title>
      ...
      <main class="flex-1 p-6 overflow-y-auto bg-slate-200">
        <div class="w-full px-4 lg:px-8 mx-auto">
          <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold text-slate-700">Data Laporan Kamar</h1>
            <a href="export-laporan-kamar.php" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow">
              <i class="fas fa-file-pdf mr-2"></i>Export PDF
            </a>
          </div>
          <div class="overflow-x-auto bg-white rounded-lg shadow">
            <?php
            // ambil data dari VIEW vw_laporan_kamar
            ...
            <table class="min-w-full table-auto divide-y divide-gray-200">
              <thead class="bg-slate-800">
                <tr>
                  ...
                  <th class="px-4 py-3 text-left text-xs font-medium text-slate-100 uppercase">Room ID</th>
                  ...
                  <th class="px-4 py-3 text-left text-xs font-medium text-slate-100 uppercase">Total Revenue</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-slate-100 uppercase">First Check-In</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-slate-100 uppercase">Last Check-Out</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                  ...
                  <td class="px-4 py-2 whitespace-nowrap font-semibold">Rp<?= number_format($row['total_revenue'], 0, ',', '.') ?></td>
                  <td class="px-4 py-2 whitespace-nowrap"><?= htmlspecialchars($row['first_check_in'] ?: '–') ?></td>
                  <td class="px-4 py-2 whitespace-nowrap"><?= htmlspecialchars($row['last_check_out'] ?: '–') ?></td>
                </tr>
                <?php endwhile; ?>
              </tbody>
            </table>
