      ...
      <main class="flex-1 p-6 overflow-y-auto overflow-x-hidden bg-slate-200">
        <div class="w-full px-4 lg:px-8 mx-auto">
          ...
          <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold text-slate-700">Data Fasilitas</h1>
            <a href="form-fasilitas.php"
            ...
            <table class="min-w-full table-auto divide-y divide-gray-200">
              <thead class="bg-slate-800">
                <tr>
                  ...
                  <th class="px-4 py-3 text-left text-xs font-medium text-slate-100 uppercase">Facility ID</th>
                  ...
                  <th class="px-4 py-3 text-right text-xs font-medium text-slate-100 uppercase">Harga</th>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <?php
                $no     = 1;
                $result = mysqli_query($connection, "SELECT * FROM tbl_facilities ORDER BY id ASC");
                while ($row = mysqli_fetch_assoc($result)):
                ?>
                <tr>
                  <td class="px-4 py-2 whitespace-nowrap"><?= $no++ ?></td>
                  <td class="px-4 py-2 whitespace-nowrap"><?= htmlspecialchars($row['facility_id']) ?></td>
                  <td class="px-4 py-2 whitespace-nowrap"><?= htmlspecialchars($row['facility_name']) ?></td>
                  <td class="px-4 py-2 overflow-hidden text-ellipsis whitespace-nowrap text-sm text-slate-600">
