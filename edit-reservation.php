...
// Jika GET[id], ambil data untuk form
    ...
    <form action="" method="POST" class="space-y-6">
      ...
      <div>
        ...
        <input type="text" readonly
               value="<?= htmlspecialchars($resData['reservation_id']) ?>"
      ...
      <div>
        ...
        <input type="text" readonly
               value="<?= htmlspecialchars($resData['customer_id'] . ' | ' . $resData['customer_name']) ?>"
               class="w-full px-4 py-2 border rounded-md bg-gray-100" />
      </div>
      ...
      <div>
        ...
        <select x-model="selectedRoom" @change="onRoomChange"
          ...
          <template x-for="r in rooms" :key="r.room_id">
            <option
              :value="`${r.room_id}||${r.room_number}||${r.type_name}`"
              :selected="r.room_id === resData.room_id"
              x-text="`${r.room_id} | ${r.room_number} | ${r.type_name}`">
            </option>
          </template>
        </select>
      </div>

      <!-- Pilih Fasilitas (required hanya jika ada opsi) -->
      <div>
        ...
        <select x-model="selectedFacility" @change="onFacilityChange"
                name="facility_id"
                x-bind:required="facilityOptions.length>0"
                x-bind:disabled="facilityOptions.length===0"
                class="w-full px-4 py-2 border rounded-md bg-white">
          <option value="">-- Pilih Fasilitas --</option>
          <template x-for="f in facilityOptions" :key="f.room_facility_id">
            <option
              :value="`${f.room_facility_id}||${f.total_price}`"
              x-text="`${f.room_facility_id} | ${f.nomor_kamar} | ${f.facility_name}`">
            </option>
          </template>
        </select>
        <div x-show="pricePerNight!==null" class="mt-2 text-lg font-semibold">
      ...
      <!-- Total & Grand Total -->
      <div x-show="nights>0" class="text-lg font-semibold">
        Total kamar (<span x-text="nights"></span> malam): Rp<span x-text="formatNumber(subTotal)"></span>
      </div>
      <div x-show="grandTotal!==null" class="text-lg font-semibold text-right">
